...
const commands = {
	...
	async maps(target, room, user, cmd) {
		if (room && toId(room.title) !== 'multiplayergames') return this.reply(`This command can only be used in the Video Games room.`);
		await pruneMaps();
		const now = Date.now();
		let index = 0;
		if (cmd === 'next') index = 1;
	...
	profile(target, room, user) {
		...
		if (!room) {
			...
			if (!isInRoom) return this.reply(`This command makes use of PM HTML boxes. Because of this, you must share a room with me that I have the bot rank in. Consider joining <<multiplayergames>>.`);
		}
